function [status]=bet_and_flirt_reg_check(feat_loc,ROI_name,mask_threshold)
...
% 
% FLIRT=['sh -c ". ${FSLDIR}etc/fslconf/fsl.sh;${FSLDIR}bin/flirt -in ',ROI_dir_name,'/ROI_mask.nii.gz',' -ref ',feat_loc,'/filtered_func_data.nii.gz',' -applyxfm -init ',ROI_dir_name,'/standard2filtered_func.mat',' -out ',ROI_dir_name,'/ROI_xfmed_mask.nii.gz"'];
% status6=system(FLIRT)
% 
Xfmation=['sh -c ". ${FSLDIR}etc/fslconf/fsl.sh;${FSLDIR}bin/fslmaths ',ROI_dir_name,'/ROI_xfmed_mask.nii.gz -thr ',num2str(0.5),' -bin ',ROI_dir_name,'/ROI_xfmed_mask_bin.nii.gz"'];
status7=system(Xfmation)
% 
...
FLIRT2=['sh -c ". ${FSLDIR}etc/fslconf/fsl.sh;${FSLDIR}bin/flirt -in ',ROI_dir_name,'/ROI_xfmed_mask_bin.nii.gz',' -ref ',' ${FSLDIR}/data/standard/MNI152_T1_2mm_brain.nii.gz ',' -applyxfm -init ',ROI_dir_name,'/filtered_func2standard.mat',' -out ',ROI_dir_name,'/ROI_again_xfmed_mask.nii.gz"'];
s = system(FLIRT2)
Xfmation_x=['sh -c ". ${FSLDIR}etc/fslconf/fsl.sh;${FSLDIR}bin/fslmaths ',ROI_dir_name,'/ROI_again_xfmed_mask.nii.gz -thr ',num2str(0.5),' -bin ',ROI_dir_name,'/ROI_again_xfmed_bin.nii.gz"'];
s2 = system(Xfmation_x)
